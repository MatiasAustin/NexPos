        ...
        <tr className="border-b border-border">
            {Array.from({ length: cols }).map((_, i) => (
                <td key={i} className="p-4">
                    <div className="h-4 bg-surface-hover rounded-lg animate-pulse" style={{ width: `${60 + Math.random() * 30}%` }} />
                </td>
            ))}
        </tr>
    );
    ...
    return (
        <div className="p-5 rounded-2xl bg-surface border border-border space-y-3 animate-pulse">
            <div className="flex justify-between items-center">
                <div className="h-5 bg-surface-hover rounded-lg w-40" />
                <div className="h-5 bg-surface-hover rounded-lg w-20" />
            </div>
...
export function LoadingSpinner({ size = 'md', text }: { size?: 'sm' | 'md' | 'lg'; text?: string }) {
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-3 py-12">
            <div className={`${sizes[size]} border-2 border-border border-t-blue-500 rounded-full animate-spin`} />
...
export function LoadingOverlay({ text = 'Memuat...' }: { text?: string }) {
    return (
        <div className="absolute inset-0 bg-background/80 flex items-center justify-center z-10 rounded-2xl backdrop-blur-sm">
            <div className="flex items-center gap-3">
                <div className="w-6 h-6 border-2 border-border border-t-blue-500 rounded-full animate-spin" />
                <span className="text-text-muted text-sm font-medium">{text}</span>
            </div>
        </div>
    );
}
