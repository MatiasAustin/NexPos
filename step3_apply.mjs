...
`;
content = content.replace(
    /(<button type="submit" disabled=\{loading\} className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-500 transition-colors mt-6">Simpan Produk<\/button>)/,
...
`;
content = content.replace(
    /(<div className="flex gap-4 mt-8">[\s\S]*?<button type="submit" disabled=\{loading\} className="flex-1 py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-500 transition-colors">Simpan Perubahan<\/button>)/,
