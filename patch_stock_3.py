...
# 1. Inline form removal
# Find the exact materialMode === 'update' inline block
inline_start = content.find('{!selectedMaterial ? (')
inline_end_search_str = '</form>\n                                                          </div>\n                                                      )}'
inline_end = content.find(inline_end_search_str, inline_start) + len(inline_end_search_str)

if inline_start != -1 and inline_end != -1 and inline_start < inline_end:
    old_inline = content[inline_start:inline_end]
    new_inline = """{!selectedMaterial && (
...
# 2. Append Material Modal before Adjust Product Stock Modal
modal_code = """
...
if "{/* Adjust Material Stock Modal */}" not in content:
